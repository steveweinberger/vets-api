      ...
      context 'with a valid GET response' do
        it 'returns a 200 with the correct schema' do
          VCR.use_cassette('vaos/systems/get_facility_clinics', match_requests_on: %i[method uri]) do
            ...
            expect(Rails.logger).to have_received(:info).with('Clinic names returned', '{"Green Team Clinic1":'\
            '{"clinicName":"CHY PC KILPATRICK",'\
            '"clinicFriendlyLocationName":"Green Team Clinic1","clinicId":"308"},'\
            '"CHY PC CASSIDY":{"clinicName":"CHY PC CASSIDY",'\
            '"clinicFriendlyLocationName":"","clinicId":"455"},'\
            '"Green Team Clinic2":{"clinicName":"MCINTYRE PC",'\
            '"clinicFriendlyLocationName":"Green Team Clinic2","clinicId":"457"},'\
            '"CHY PC VAR2":{"clinicName":"CHY PC VAR2",'\
            '"clinicFriendlyLocationName":"CHY PC VAR2","clinicId":"848"}}').at_least(:once)
            expect(response).to have_http_status(:ok)
